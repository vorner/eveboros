...
impl Message {
    fn get<T: Any + 'static + Send>(self) -> Result<T> {
...
mod tests {
    ...
    impl Event<(), RemoteRecipient> for RemoteRecipient {
        ...
        fn message<S: Scope<(), RemoteRecipient>>(&mut self, scope: &mut S, message: Message) -> Response {
            let RemoteHello(mut sender) = message.get::<RemoteHello>()?;
            sender.send(()).unwrap();
            Ok(false)
        }
    }
